...
async def test_get_logs_with_limit(
    ...
    """
    pipeline_name = "test-logs-limit-pipeline"
    ...
    logs_limit_5 = await pipeline_resource.get_logs(pipeline_name=pipeline_name, limit=5)
    logs_limit_10 = await pipeline_resource.get_logs(pipeline_name=pipeline_name, limit=10)
    
    assert isinstance(logs_limit_5, PipelineLogList)
    assert isinstance(logs_limit_10, PipelineLogList)
    
    # If there are logs, the limit should be respected
    if logs_limit_5.data:
        assert len(logs_limit_5.data) <= 5
    if logs_limit_10.data:
        assert len(logs_limit_10.data) <= 10
...
async def test_get_logs_with_level_filter(
    ...
    """
    pipeline_name = "test-logs-level-pipeline"
    ...
    # Test getting logs for different levels
    info_logs = await pipeline_resource.get_logs(pipeline_name=pipeline_name, level=LogLevel.INFO)
    warning_logs = await pipeline_resource.get_logs(pipeline_name=pipeline_name, level=LogLevel.WARNING)
    error_logs = await pipeline_resource.get_logs(pipeline_name=pipeline_name, level=LogLevel.ERROR)
    
    # Verify all responses are valid
    assert isinstance(info_logs, PipelineLogList)
    assert isinstance(warning_logs, PipelineLogList)
    assert isinstance(error_logs, PipelineLogList)
    
    # If there are logs, they should match the requested level
    for log_entry in info_logs.data:
        assert log_entry.level.lower() == LogLevel.INFO.lower()
    
    for log_entry in warning_logs.data:
        assert log_entry.level.lower() == LogLevel.WARNING.lower()
        
    for log_entry in error_logs.data:
        assert log_entry.level.lower() == LogLevel.ERROR.lower()
