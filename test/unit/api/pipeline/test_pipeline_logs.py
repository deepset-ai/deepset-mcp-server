...
class TestPipelineHandleGetLogs:
    """Tests for the get_logs method on PipelineHandle."""

    @pytest.mark.asyncio
    async def test_handle_get_logs_default_params(self) -> None:
        """Test getting logs through PipelineHandle with default parameters."""
        # Create sample logs
        sample_logs = [
            create_sample_log(log_id="log1", message="First log entry"),
            create_sample_log(log_id="log2", message="Second log entry"),
        ...
        resource = PipelineResource(client=client, workspace="test-workspace")
        handle = PipelineHandle(pipeline=pipeline, resource=resource)

        # Call get_logs method
        result = await handle.get_logs()
        ...
        # Verify request used the pipeline name from the handle
        assert client.requests[0]["endpoint"] == "v1/workspaces/test-workspace/pipelines/test-pipeline/logs"
        assert client.requests[0]["params"] == {"limit": 30, "filter": "origin eq 'querypipeline'"}

    @pytest.mark.asyncio
    async def test_handle_get_logs_with_custom_params(self) -> None:
        """Test getting logs through PipelineHandle with custom parameters."""
        # Create sample error logs
        sample_logs = [
            create_sample_log(log_id="error1", message="First error", level="error"),
        ]
        ...
        # Call get_logs method with custom parameters
        result = await handle.get_logs(limit=10, level="error")

        # Verify results
        assert len(result.data) == 1
        assert result.data[0].level == "error"

        # Verify request with custom parameters
        assert client.requests[0]["params"] == {"limit": 10, "filter": "level eq 'error' and origin eq 'querypipeline'"}

    @pytest.mark.asyncio
    async def test_handle_get_logs_error(self) -> None:
        """Test error handling when getting logs through PipelineHandle."""
        # Create client that raises an exception
        client = DummyClient(responses={"test-workspace/pipelines/test-pipeline/logs": ValueError("API Error")})
