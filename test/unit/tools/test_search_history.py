...
async def test_list_search_history_empty() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_response=PaginatedResponse(data=[], has_more=False, total=0)
    )
    client = FakeClient(resource)
...
async def test_list_search_history_with_pagination() -> None:
    ...
    response = PaginatedResponse(
        data=[entry], has_more=True, total=10, next_cursor="cursor_abc"
    )

    resource = FakeSearchHistoryResource(list_response=response)
    client = FakeClient(resource)

    result = await list_search_history(
        client=client, workspace="ws1", limit=5, after="cursor_prev"
    )
...
async def test_list_search_history_bad_request() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_exception=BadRequestError(message="Invalid parameters")
    )
    client = FakeClient(resource)
...
async def test_list_pipeline_search_history_returns_entries() -> None:
    ...
    entry1 = create_search_history_entry(
        query="pipeline query 1", pipeline_name="my-pipeline"
    )
    entry2 = create_search_history_entry(
        query="pipeline query 2", pipeline_name="my-pipeline"
    )
    response = PaginatedResponse(data=[entry1, entry2], has_more=False, total=2)
    ...
    result = await list_pipeline_search_history(
        client=client, workspace="ws1", pipeline_name="my-pipeline"
    )
...
async def test_list_pipeline_search_history_empty() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_pipeline_response=PaginatedResponse(data=[], has_more=False, total=0)
    )
    client = FakeClient(resource)

    result = await list_pipeline_search_history(
        client=client, workspace="ws1", pipeline_name="empty-pipeline"
    )
...
async def test_list_pipeline_search_history_with_pagination() -> None:
    ...
    entry = create_search_history_entry(
        query="paged pipeline query", pipeline_name="p"
    )
    response = PaginatedResponse(
        data=[entry], has_more=True, total=5, next_cursor="next_page"
    )
...
async def test_list_pipeline_search_history_pipeline_not_found() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_pipeline_exception=ResourceNotFoundError()
    )
    client = FakeClient(resource)

    result = await list_pipeline_search_history(
        client=client, workspace="ws1", pipeline_name="missing-pipeline"
    )
...
async def test_list_pipeline_search_history_bad_request() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_pipeline_exception=BadRequestError(message="Invalid filter")
    )
    client = FakeClient(resource)
...
async def test_list_pipeline_search_history_unexpected_error() -> None:
    ...
    resource = FakeSearchHistoryResource(
        list_pipeline_exception=UnexpectedAPIError(
